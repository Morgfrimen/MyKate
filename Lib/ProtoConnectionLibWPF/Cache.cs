...
namespace ProtoConnectionLibWPF
{
    internal static class Cache
    {
        static Cache()
        {
            ...
            ReadOrWriteSettings(configFile,settings,nameof(UrlServer), "https://localhost:5001",ref _urlServer);
            ReadOrWriteSettings(configFile, settings, nameof(GuidUser), Guid.NewGuid().ToString(), ref _guidUser);
        ...
        private static void ReadOrWriteSettings(Configuration configFile, KeyValueConfigurationCollection settings,string key,string defaultValue, ref string field)
        {
        ...
        private static void ReadOrWriteSettings(Configuration configFile, KeyValueConfigurationCollection settings, string key, string defaultValue, ref Guid field)
        {
